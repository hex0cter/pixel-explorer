...
function initPixelZoom() {
  ...
  console.log("Zoom Level Display:", zoomLevelDisplay);
  ...
  // Update zoom level display with initial value
  ...
  zoomSlider.addEventListener("input", function () {
    ...
    // Update zoom level display with new value
    zoomLevelDisplay.textContent = currentZoomLevel;
  ...
  function updateZoomedView() {
    ...
    console.log(
      ...
      currentZoomLevel
        ...
        // Default: enable image smoothing (for drawImage)
        zoomCtx.imageSmoothingEnabled = true;

        try {
      const maxZoomLevel = 40; // Maximum zoom level from the slider

      // For maximum zoom level, render a single pixel with RGB subpixels
      if (currentZoomLevel === maxZoomLevel) {
        console.log(
          "Using RGB subpixel rendering for maximum zoom (single pixel)"
        );

        // Source coordinates in the original image (just one pixel at the center)
        const sourceX = Math.max(0, Math.min(canvas.width - 1, imgX));
        const sourceY = Math.max(0, Math.min(canvas.height - 1, imgY));
        ...
        // Size of the pixel in the zoomed view (full canvas)
        const pixelSize = zoomedView.width;
        const subpixelWidth = pixelSize / 3; // Each subpixel is 1/3 the width of the full pixel

        // Draw three rectangles to represent RGB subpixels of a single pixel
        // Red subpixel (leftmost)
        zoomCtx.fillStyle = `rgb(${red}, 0, 0)`;
        zoomCtx.fillRect(0, 0, subpixelWidth, pixelSize);

        // Green subpixel (middle)
        zoomCtx.fillStyle = `rgb(0, ${green}, 0)`;
        zoomCtx.fillRect(subpixelWidth, 0, subpixelWidth, pixelSize);

        // Blue subpixel (rightmost)
        zoomCtx.fillStyle = `rgb(0, 0, ${blue})`;
        zoomCtx.fillRect(subpixelWidth * 2, 0, subpixelWidth, pixelSize);

        // Draw separators between subpixels
        zoomCtx.strokeStyle = "#888";
        zoomCtx.lineWidth = 1;
        zoomCtx.beginPath();
        zoomCtx.moveTo(subpixelWidth, 0);
        zoomCtx.lineTo(subpixelWidth, pixelSize);
        zoomCtx.stroke();

        zoomCtx.beginPath();
        zoomCtx.moveTo(subpixelWidth * 2, 0);
        zoomCtx.lineTo(subpixelWidth * 2, pixelSize);
        zoomCtx.stroke();

        // Draw outline around the pixel
        zoomCtx.strokeStyle = "#888";
        zoomCtx.lineWidth = 2;
        zoomCtx.strokeRect(0, 0, pixelSize, pixelSize);
      }
      // For high zoom levels (but not maximum), use drawImage with nearest-neighbor scaling
      else if (currentZoomLevel > 10) {
        console.log("Using drawImage with nearest-neighbor scaling for high zoom");

        // Size of the area to sample from the original image (radius from center)
        const sampleRadius = baseSampleSize / Math.max(1, currentZoomLevel);
        // Calculate the source rectangle dimensions (diameter)
        const sourceSize = Math.max(1, Math.round(sampleRadius * 2)); // Ensure at least 1 pixel

        // Calculate the top-left corner of the source rectangle
        const sourceX = Math.max(0, Math.round(imgX - sampleRadius));
        const sourceY = Math.max(0, Math.round(imgY - sampleRadius));

        // Ensure source dimensions don't exceed canvas bounds
        const sourceWidth = Math.min(sourceSize, canvas.width - sourceX);
        const sourceHeight = Math.min(sourceSize, canvas.height - sourceY);

        console.log(
          `Zoom ${currentZoomLevel}: Sample Radius ${sampleRadius.toFixed(2)}, Source Rect: (${sourceX}, ${sourceY}, ${sourceWidth}, ${sourceHeight})`
        );

        // Disable image smoothing to get sharp, pixelated scaling
        zoomCtx.imageSmoothingEnabled = false;

        // Draw the sampled portion to the zoomed view, scaling it up
        zoomCtx.drawImage(
          canvas,       // Source image canvas
          sourceX,      // Source x
          sourceY,      // Source y
          ...
          0,            // Destination x
          0,            // Destination y
          zoomedView.width,  // Destination width
          zoomedView.height // Destination height
        );

        // Draw grid lines ONLY when zoom level is greater than 20
        if (currentZoomLevel > 20) {
            const numPixelsAcross = sourceWidth; // Use sourceWidth as the basis for grid lines
            const displayPixelSize = zoomedView.width / numPixelsAcross;
            zoomCtx.strokeStyle = "#888";
            zoomCtx.lineWidth = 0.5;
            for (let i = 1; i < numPixelsAcross; i++) {
                const pos = i * displayPixelSize;
                // Vertical lines
                zoomCtx.beginPath();
                zoomCtx.moveTo(pos, 0);
                zoomCtx.lineTo(pos, zoomedView.height);
                zoomCtx.stroke();
                // Horizontal lines
                zoomCtx.beginPath();
                zoomCtx.moveTo(0, pos);
                zoomCtx.lineTo(zoomedView.width, pos);
                zoomCtx.stroke();
            }
        }

        // Re-enable image smoothing for subsequent draws if needed (though the next branch does it)
        zoomCtx.imageSmoothingEnabled = true;

      } else {
        // For lower zoom levels, use drawImage with default smoothing
        console.log("Using drawImage with default smoothing for low zoom");

        // Calculate the source rectangle to sample from the original image (using sampleSize from outer scope)
